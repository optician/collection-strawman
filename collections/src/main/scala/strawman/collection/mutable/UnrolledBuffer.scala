...
import scala.{Int, Unit, Array, Serializable, SerialVersionUID, transient, IndexOutOfBoundsException}
import scala.Predef.{assert, String}
    ...
    with Serializable
{
  ...
  def patchInPlace(from: Int, patch: strawman.collection.Seq[T], replaced: Int): this.type = {
...
object UnrolledBuffer extends StrictOptimizedClassTagSeqFactory[UnrolledBuffer] { self =>
  ...
  def from[A : ClassTag](source: strawman.collection.IterableOnce[A]): UnrolledBuffer[A] = newBuilder[A]().addAll(source)
  ...
  class Unrolled[T: ClassTag] private[collection] (var size: Int, var array: Array[T], var next: Unrolled[T], val buff: UnrolledBuffer[T] = null) {
    ...
    @tailrec final def insertAll(idx: Int, t: strawman.collection.IterableOnce[T], buffer: UnrolledBuffer[T]): Int = {
      if (idx < size) {
